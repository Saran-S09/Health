    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      <div>
        ...
        <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)' }}>
          Manage incoming requests & search patients to send monitoring requests
        </p>
      </div>

      {/* Doctor Search & Add Patient Section */}
      <div className="glass-panel" style={{ padding: '1.5rem', backgroundColor: '#ffffff' }}>
        <h3 style={{ fontSize: '1.05rem', fontWeight: 800, color: 'var(--text-main)', marginBottom: '0.75rem' }}>
          Search Available Patients & Send Connection Request
        </h3>
        <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '1rem' }}>
          Enter Patient ID (e.g. CL-P10234) or Patient Name to request care-team authorization:
        </p>
        ...
        {sentSuccess ? (
          <div style={{ padding: '0.75rem', backgroundColor: 'var(--status-normal-bg)', borderRadius: '8px', color: '#059669', fontSize: '0.85rem', fontWeight: 700 }}>
            <CheckCircle2 size={16} style={{ display: 'inline', marginRight: '6px' }} /> Connection request dispatched to patient. Status: PENDING approval.
          </div>
        ) : (
      ...
      <div>
        <h3 style={{ fontSize: '1.05rem', fontWeight: 800, color: 'var(--text-main)', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <UserCheck size={18} color="var(--primary)" /> Patient Requests Received
        </h3>
